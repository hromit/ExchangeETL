...
// https://www.moneycontrol.com/news/tags/coronavirus.html/page-3/

class MoneyControlNews extends TestCrawlerTrait{
  ...
  override protected def doProcess(page: Page): Unit = {
    ...
    val cleanedHTML = Jsoup.clean(page.body.toString, Whitelist.none())

    println(cleanedHTML)



    val result = FileDTO(fileName = s"MoneyControlNewsPolitics${dateSelected}",content = page.body.toString)
