...
class MoneyControlTopGainersPageProcessor  extends TestCrawlerTrait{
  ...
  override protected def doProcess(page: Page): Unit = {
    ...
    val result = FileDTO(fileName = s"MoneyControlTopGainers${dateSelected}",content = cleanedHTML.toString)
