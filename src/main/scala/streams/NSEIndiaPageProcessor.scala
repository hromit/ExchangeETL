...
class NSEIndiaPageProcessor extends TestCrawlerTrait{

  override def targetRequests: List[RequestUri] = {
    println("NSEIndiaPageProcessor TARGETREQUESTS")
    List(
      RequestUri("https://www.nseindia.com/")
    )}

  override def requestSetting: RequestSetting = {
    println("NSEIndiaPageProcessor REQUESTSETTING")
    RequestSetting(
      domain = "nseindia.com",
      sleepTime = 1 seconds,
      xhrRequests = Set("https://www.nseindia.com/")
    )}
  ...
  override protected def doProcess(page: Page): Unit = {
    ...
    val result = FileDTO(fileName = s"NSEIndiaCrawlResults${dateSelected}",content = page.body.toString)
