...
class BSEIndiaPageProcessor extends TestCrawlerTrait{

  override def targetRequests: List[RequestUri] = {
    println("BSEIndiaPageProcessor TARGETREQUESTS")
    List(
      RequestUri("https://www.bseindia.com/")
    )}

  override def requestSetting: RequestSetting = {
    println("BSEIndiaPageProcessor REQUESTSETTING")
    RequestSetting(
      domain = "bseindia.com",
      sleepTime = 1 seconds,
      xhrRequests = Set("https://www.bseindia.com/")
    )}

  override protected def doProcess(page: Page): Unit = {
    println("BSEIndiaPageProcessor PAGE ==> " + page)
    println("BSEIndiaPageProcessor RESULTITEMS SIZE ==>" + page.resultItems.size())
    println("BSEIndiaPageProcessor PAGE URL ==> " + page.url)
    println("BSEIndiaPageProcessor BODY ==> ")
    println("BSEIndiaPageProcessor PAGE TITLE ==> "+page.title)
    println("BSEIndiaPageProcessor DOWNLOADER => "+downloader)
    ...
    val result = FileDTO(fileName = s"BSEIndiaCrawlResults${dateSelected}",content = page.body.toString)
