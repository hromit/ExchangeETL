...
class ThePrintPageProcessor extends TestCrawlerTrait{

  override def targetRequests: List[RequestUri] = {
    println("=== inside qunar target req")
    List(
      RequestUri("https://theprint.in/")
    )}
  ...
  override def requestSetting: RequestSetting = {
    ...
    RequestSetting(
      ...
      sleepTime = 1 seconds,
      xhrRequests = Set("https://theprint.in/")
    )}
  ...
  override protected def doProcess(page: Page): Unit = {
    ...
    page.hrefs.foreach{ s  => if (s.mkString.contains("covid")) println(s)}

    page.hrefs.filter(_.mkString.contains("covid")).foreach{ println}
